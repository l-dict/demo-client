    ...
    <div>
      ...
      <div class="row pi-draggable" draggable="true">
        ...
        <div class="col-md-6 d-flex w-100 justify-content-left">
          ...
          <a href="" class="nav-link " href="javascript:void(0)" data-placement="top" data-toggle="modal" title="删除选中" data-target="#deletcheckModal" @click="delCollection(item)"><i class="fa fa-calendar-times-o fa-fw"></i>删除已选</a>
      ...
      <div>
        ...
        <!-- 重复部分 -->
          <div class="col-md-6 border-bottom pr-3 pl-0"  v-for="(item, index) in collectionList">
            <ul class=" d-flex w-100 justify-content-between list-inline pi-draggable m-1 ">
              <li class="list-inline-item col-1" >{{index}}</li>
              <li class="list-inline-item col-6 text-left p-0" style="width: 300px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">{{item.word}}&nbsp;:&nbsp;{{item.translation}}</li>
              <li class="list-inline-item col-1 ">
              <a class="btn btn-outline-primary border-0 " href="javascript:void(0)" data-placement="top" data-toggle="modal" title="删除" data-target="#deletcheckModal" @click="delCollectionConfirm(item)"><i class="fa fa-fw nav-link fa-bitbucket p-0"></i></a></li>
      ...
      <div  class="modal" id="deletcheckModal">
        <div class="modal-dialog align-items-center mt-5"   role="document" style="width: 300px;">
          <div class="modal-content">
            ...
            <div class="modal-footer border-0"> <button type="button" class="btn btn-primary" data-dismiss="modal" @click="delCollection">Yes,delet</button> <button type="button" class="btn btn-secondary" data-dismiss="modal">No,cancel</button> </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods:{
    init(){
      axios.get("/users/collectionList").then((response)=>{
        let res = response.data;
        if(res.status==0){
          if(res.result.count==0){
             this.msg="朋友，你还没收藏任何单词！";
          }else{
            this.flag=true;
            this.collectionList = res.result.collectionList;
          }
        }else{
          this.msg="抱歉，出错啦！";
        }
    ...
    delCollectionConfirm(item){
                this.delItem = item;
                //this.modalConfirm = true;
            },
    delCollection(){
              axios.post("/users/collectionDel",{
                word:this.delItem.word,
              }).then((response)=>{
                  let res = response.data;
                  if(res.status == '0'){
                    alert("删除成功！");
